...
# Silence torch elastic redirect warnings on Windows/macOS
logging.getLogger("torch.distributed.elastic.multiprocessing.redirects").setLevel(logging.ERROR)

# Suppress noisy WinError 10054 asyncio transport messages on Windows
if sys.platform == "win32":
    ...
    # As a fallback, suppress WinError 10054 from Proactor transport shutdown
    try:
        ...
        _pe._ProactorBasePipeTransport._call_connection_lost = _quiet_call_connection_lost
    except Exception:
        pass
...
def main():
    ...
    try:
        ...
        # Allow access to files outside gradio cache (needed for local audio paths)
        allowed_paths = [output_dir, project_root]
        if os.name == "nt":
            allowed_paths.append("C:\\")

        # Ensure WinError 10054 noise is suppressed on the active loop too
        if sys.platform == "win32":
            try:
