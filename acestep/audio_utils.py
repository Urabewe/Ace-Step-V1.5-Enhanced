...
class AudioSaver:
    ...
    def save_audio(
        ...
        try:
            if format in ["mp3", "flac"] and metadata:
                ffmpeg_path = shutil.which("ffmpeg")
                if ffmpeg_path:
                    # Write temp WAV via soundfile, then transcode with ffmpeg and embed metadata
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
                        tmp_path = tmp.name
                    try:
                        audio_np = audio_tensor.transpose(0, 1).numpy()  # [samples, channels]
                        sf.write(tmp_path, audio_np, sample_rate, format="WAV")
                    ...
                    logger.warning("[AudioSaver] FFmpeg not found; saving without metadata.")
                    torchaudio.save(
                        str(output_path),
                        audio_tensor,
                        sample_rate,
                        channels_first=True,
                        backend='soundfile' if format == "flac" else 'ffmpeg',
                    )
            elif format == "mp3":
                ffmpeg_path = shutil.which("ffmpeg")
                if ffmpeg_path:
                    # Write temp WAV via soundfile, then transcode to MP3 with ffmpeg
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
                        tmp_path = tmp.name
                    try:
                        audio_np = audio_tensor.transpose(0, 1).numpy()  # [samples, channels]
                        sf.write(tmp_path, audio_np, sample_rate, format="WAV")
                        subprocess.run(
                            [ffmpeg_path, "-y", "-i", tmp_path, str(output_path)],
                            check=True,
                            stdout=subprocess.DEVNULL,
                            stderr=subprocess.DEVNULL,
                        )
    ...
    def convert_audio(
        ...
        # Load audio (soundfile preferred to avoid torchcodec issues)
        data, sample_rate = sf.read(str(input_path), always_2d=True, dtype="float32")
        audio_tensor = torch.from_numpy(data).transpose(0, 1)
