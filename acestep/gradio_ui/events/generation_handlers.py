...
def init_service_wrapper(dit_handler, llm_handler, config_path, device, init_llm, lm_model_path, backend, use_flash_attention, offload_to_cpu, offload_dit_to_cpu, compile_model):
    ...
    if init_llm:
        ...
        lm_dl_ok, lm_dl_msg = ensure_lm_model(lm_model_path, checkpoint_dir)
        if not lm_dl_ok:
            status += f"\n{lm_dl_msg}"
        else:
            status += f"\n{lm_dl_msg}"
...
def update_audio_components_visibility(batch_size):
    ...
    try:
        ...
        # Leave UI unchanged while user is editing the field.
        return (gr.skip(),) * 9
