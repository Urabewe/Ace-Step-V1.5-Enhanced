...
# Cache large tensors server-side to avoid bloating Gradio state
_EXTRA_OUTPUTS_CACHE: Dict[int, Dict[str, Any]] = {}


def _cache_extra_outputs(batch_index: int, extra_outputs: Optional[Dict[str, Any]]) -> None:
    if isinstance(extra_outputs, dict) and extra_outputs:
        _EXTRA_OUTPUTS_CACHE[batch_index] = extra_outputs


def _get_cached_extra_outputs(batch_index: int) -> Dict[str, Any]:
    return _EXTRA_OUTPUTS_CACHE.get(batch_index, {})


def _sanitize_metadata_for_audio(metadata: Dict[str, Any]) -> Dict[str, Any]:
    """Remove path-like fields before embedding into audio metadata."""
    if not isinstance(metadata, dict):
        return {}

    def is_path_value(value: Any) -> bool:
    ...
    for key, value in metadata.items():
        ...
        if "path" in key_lower or "file" in key_lower or "filename" in key_lower:
            continue
        if is_path_value(value):
            continue
        if isinstance(value, list):
            filtered_list = [v for v in value if not is_path_value(v)]
            cleaned[key] = filtered_list
            continue
        if isinstance(value, dict):
            cleaned[key] = _sanitize_metadata_for_audio(value)
            continue
        cleaned[key] = value
    return cleaned


def _coerce_batch_size(value: Any, default: int = 1) -> int:
    try:
...
def store_batch_in_queue(
    ...
    """
    _cache_extra_outputs(batch_index, extra_outputs)
    batch_queue[batch_index] = {
        ...
        "extra_outputs": {},  # Keep large tensors out of Gradio state
...
def generate_with_progress(
    ...
    use_adg, cfg_interval_start, cfg_interval_end, shift, infer_method, custom_timesteps, audio_format, autosave_outputs,
    autosave_dir,
    lm_temperature, cache_src_audio,
    ...
    # Limits removed to allow full user control
    if cache_src_audio is False:
        try:
            dit_handler._update_src_audio_cache(None)
        except Exception:
            pass
    ...
    # Normalize batch size early to avoid UI errors on empty input
    batch_size_input = _coerce_batch_size(batch_size_input, default=1)
    ...
    autosave_dir = autosave_dir or DEFAULT_RESULTS_DIR
    autosave_dir = os.path.abspath(autosave_dir).replace("\\", "/")
    date_dir = os.path.join(autosave_dir, datetime.date.today().strftime("%Y-%m-%d")).replace("\\", "/")
    for i in range(8):
        if i < len(audios):
            key = audios[i]["key"]
...
def generate_lrc_handler(dit_handler, sample_idx, current_batch_index, batch_queue, vocal_language, inference_steps):
    ...
    batch_data = batch_queue[current_batch_index]
    extra_outputs = batch_data.get("extra_outputs", {}) or _get_cached_extra_outputs(current_batch_index)
...
def generate_next_batch_background(
    ...
    try:
        ...
        params.setdefault("autosave_outputs", False)
        params.setdefault("autosave_dir", "")
        ...
        generator = generate_with_progress(
            ...
            autosave_outputs=params.get("autosave_outputs"),
            autosave_dir=params.get("autosave_dir"),
