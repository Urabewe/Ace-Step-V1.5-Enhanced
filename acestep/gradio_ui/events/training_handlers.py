...
def _normalize_training_state(training_state: Any) -> Dict:
    """Ensure training_state is a mutable dict even if Gradio passes a string."""
    if isinstance(training_state, dict):
        return training_state
    if isinstance(training_state, str):
        try:
            parsed = json.loads(training_state)
...
def get_sample_preview(
    ...
    display_lyrics = (
        sample.lyrics
        if sample.lyrics
        else (sample.formatted_lyrics if sample.formatted_lyrics else sample.raw_lyrics)
    )
...
def reset_lyrics_from_txt(
    sample_idx: int,
    builder_state: Optional[DatasetBuilder],
):
    """Reset the lyrics field from the raw .txt lyrics file if available."""
    if builder_state is None or not builder_state.samples:
        return gr.update(), gr.update(), gr.update(), "❌ No dataset loaded", builder_state

    if sample_idx is None:
        return gr.update(), gr.update(), gr.update(), "❌ No sample selected", builder_state

    idx = int(sample_idx)
    if idx < 0 or idx >= len(builder_state.samples):
        return gr.update(), gr.update(), gr.update(), f"❌ Invalid sample index: {idx}", builder_state

    sample = builder_state.samples[idx]
    if not sample.has_raw_lyrics():
        return (
            gr.update(value=sample.lyrics or ""),
            gr.update(value=sample.is_instrumental),
            gr.update(value=sample.language),
            "⚠️ No .txt lyrics found for this sample",
            builder_state,
        )

    raw = (sample.raw_lyrics or "").strip()
    if not raw:
        return (
            gr.update(value=sample.lyrics or ""),
            gr.update(value=sample.is_instrumental),
            gr.update(value=sample.language),
            "⚠️ .txt lyrics file is empty",
            builder_state,
        )
...
def load_existing_dataset_for_preprocess(
    ...
    """
    # Empty preview: (audio_path, filename, caption, genre, prompt_override, lyrics, bpm, keyscale, timesig, duration, language, instrumental)
    empty_preview = (None, "", "", "", "Use Global Ratio", "", None, "", "", 0.0, "instrumental", True)
    ...
    display_lyrics = (
        first_sample.lyrics
        if first_sample.lyrics
        else (first_sample.formatted_lyrics if first_sample.formatted_lyrics else first_sample.raw_lyrics)
    )
...
def preprocess_dataset(
    ...
    def progress_callback(current, total, filename, elapsed=None, file_elapsed=None):
        if not progress:
            return
        try:
            ...
            if file_elapsed is not None:
                per_file = f" | file {file_elapsed:,.1f}s"
            progress(ratio, f"Preprocessing {current}/{total}: {filename}{per_file}{eta}")
        except Exception:
            pass
...
def start_training(
    ...
    if training_source == "raw":
        if not raw_audio_dir or not raw_audio_dir.strip():
            ...
            return
        if not tensor_dir or not tensor_dir.strip():
            yield "❌ Please enter a tensor output directory", "", None, training_state
            return

        raw_audio_dir = raw_audio_dir.strip()
        tensor_dir = tensor_dir.strip()

        builder = DatasetBuilder()
        builder.metadata.name = os.path.basename(os.path.normpath(raw_audio_dir)) or "raw_audio"
        builder.metadata.custom_tag = (raw_custom_tag or "").strip()
        builder.metadata.tag_position = raw_tag_position or "prepend"
        builder.metadata.all_instrumental = True

        samples, status = builder.scan_directory(raw_audio_dir)
        if not samples:
        ...
        if raw_auto_label:
            if llm_handler is None or not getattr(llm_handler, "llm_initialized", False):
                yield "❌ 5Hz LM not initialized. Initialize LM or disable auto-label.", "", None, training_state
                return
            ...
            for sample in builder.samples:
                ...
                caption = filename.replace("_", " ").replace("-", " ").strip()
                sample.caption = caption
                sample.labeled = True

        if builder.get_labeled_count() == 0:
            yield "❌ No labeled samples found after scanning", "", None, training_state
            return

        yield "🔄 Preprocessing raw audio to tensors...", "", None, training_state
        _, preprocess_status = builder.preprocess_to_tensors(
            dit_handler=dit_handler,
            output_dir=tensor_dir,
            progress_callback=progress_callback,
        )
        if "❌" in preprocess_status:
            yield preprocess_status, "", None, training_state
            return
    else:
        if not tensor_dir or not tensor_dir.strip():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to set matmul precision ({matmul_precision}): {e}")

        if torch.cuda.is_available():
            # Avoid mixing legacy matmul TF32 toggles with set_float32_matmul_precision
        ...
        for step, loss, status in trainer.train_from_preprocessed(
            ...
            elapsed_seconds = base_elapsed_seconds + (time.time() - start_time)
            training_state["elapsed_seconds"] = elapsed_seconds
            ...
            if training_state.get("should_stop", False):
                logger.info("⏸️ Training pause requested by user")
                pause_msg = "⏸️ Pause requested. Saving checkpoint..."
                checkpoint_dir = training_state.get("last_checkpoint_dir")
                if checkpoint_dir:
                    pause_msg += f" ({checkpoint_dir})"
                log_lines.append(pause_msg)
                yield f"{pause_msg} ({time_info})", "\n".join(log_lines[-15:]), loss_data, training_state
                break
        
        total_time = base_elapsed_seconds + (time.time() - start_time)
...
def stop_training(training_state: Dict) -> Tuple[str, Dict]:
    ...
    return "⏸️ Pausing training (saving checkpoint)...", training_state
