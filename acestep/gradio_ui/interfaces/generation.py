...
from acestep.gradio_ui.ui_settings import load_ui_settings
...
def create_generation_section(dit_handler, llm_handler, init_params=None, language='en') -> dict:
    ...
    autosave_outputs_value = ui_settings.get("autosave_outputs", True)
    autosave_dir_value = ui_settings.get("autosave_dir") or "./gradio_outputs"
    ...
    with gr.Group():
        ...
        with gr.Accordion(t("service.title"), open=accordion_open, visible=accordion_visible) as service_config_accordion:
            ...
            with gr.Row():
                ...
                default_lm_model = "acestep-5Hz-lm-1.7B" if "acestep-5Hz-lm-1.7B" in available_lm_models else (available_lm_models[0] if available_lm_models else None)
            ...
            with gr.Row():
                ...
                lora_scale_slider = gr.Slider(
                    ...
                    step=0.05,
                    label="LoRA Scale",
                    info="LoRA influence strength (0=disabled, 1=full, 2=extra)",
                    scale=2,
                )
        ...
        with gr.Accordion(t("generation.advanced_settings"), open=False):
            ...
            with gr.Row():
                autosave_dir = gr.Textbox(
                    label=t("generation.autosave_dir_label"),
                    value=autosave_dir_value,
                    placeholder="./gradio_outputs",
                    info=t("generation.autosave_dir_info"),
                )
        ...
        # Small options row above Generate
        with gr.Row(equal_height=True):
            ...
            autosave_outputs = gr.Checkbox(
                label=t("generation.autosave_outputs_label"),
                value=autosave_outputs_value,
                info=t("generation.autosave_outputs_info"),
                interactive=True,
            )
            cache_src_audio = gr.Checkbox(
                label=t("generation.cache_src_audio_label"),
                value=True,
                info=t("generation.cache_src_audio_info"),
            )
    ...
    return {
        ...
        "ui_settings_state": gr.State({}),
        "cache_src_audio": cache_src_audio,
