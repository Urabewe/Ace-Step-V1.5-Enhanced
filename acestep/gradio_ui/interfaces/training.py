...
def create_training_section(dit_handler, llm_handler, init_params=None) -> dict:
            ...
            with gr.Tab(t("training.tab_train_lora")):
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        training_source = gr.Radio(
                            choices=[
                                (t("training.source_preprocessed"), "preprocessed"),
                                (t("training.source_raw"), "raw"),
                            ],
                            value="preprocessed",
                            label=t("training.training_source"),
                            info=t("training.training_source_info"),
                        )
                        ...
                        raw_audio_dir = gr.Textbox(
                            label=t("training.raw_audio_dir"),
                            placeholder="./datasets/raw_audio",
                            info=t("training.raw_audio_info"),
                            visible=False,
                        )

                        raw_custom_tag = gr.Textbox(
                            label=t("training.raw_custom_tag"),
                            placeholder="e.g., 8bit_retro, my_style",
                            info=t("training.raw_custom_tag_info"),
                            visible=False,
                        )

                        raw_tag_position = gr.Radio(
                            choices=[
                                (t("training.tag_prepend"), "prepend"),
                                (t("training.tag_append"), "append"),
                                (t("training.tag_replace"), "replace"),
                            ...
                            value="prepend",
                            label=t("training.raw_tag_position"),
                            info=t("training.raw_tag_position_info"),
                            visible=False,
                        )

                        raw_auto_label = gr.Checkbox(
                            label=t("training.raw_auto_label"),
                            value=False,
                            info=t("training.raw_auto_label_info"),
                            visible=False,
                        )
                        ...
                        def _toggle_training_source(source: str):
                            is_raw = source == "raw"
                            return (
                                gr.update(visible=not is_raw),
                                gr.update(visible=not is_raw),
                                gr.update(visible=is_raw),
                                gr.update(visible=is_raw),
                                gr.update(visible=is_raw),
                                gr.update(visible=is_raw),
                                gr.update(
                                    label=t("training.tensor_output_dir") if is_raw else t("training.preprocessed_tensors_dir"),
                                    info=t("training.tensor_output_info") if is_raw else t("training.preprocessed_tensors_info"),
                                ),
                            )

                        training_source.change(
                            fn=_toggle_training_source,
                ...
                with gr.Row():
                    num_workers = gr.Slider(
                        ...
                        step=1,
                        value=4,
                        label=t("training.num_workers"),
                        info=t("training.num_workers_info"),
                    )

                    prefetch_factor = gr.Slider(
                        minimum=1,
                        maximum=8,
                        step=1,
                        value=2,
                        label=t("training.prefetch_factor"),
                        info=t("training.prefetch_factor_info"),
                    )

                    persistent_workers = gr.Checkbox(
                        label=t("training.persistent_workers"),
                        value=False,
                        info=t("training.persistent_workers_info"),
                    )

                    pin_memory = gr.Checkbox(
                        label=t("training.pin_memory"),
                        value=True,
                        info=t("training.pin_memory_info"),
                    )
                ...
                with gr.Row():
                    matmul_precision = gr.Dropdown(
                        ...
                        value="high",
                        label=t("training.matmul_precision"),
                        info=t("training.matmul_precision_info"),
                    )

                    allow_tf32 = gr.Checkbox(
                        label=t("training.allow_tf32"),
                        value=True,
                        info=t("training.allow_tf32_info"),
                    )

                    compile_training = gr.Checkbox(
                ...
                with gr.Row():
                    preset_low_btn = gr.Button(
                        t("training.preset_low"),
                        variant="secondary",
                    )
                    preset_medium_btn = gr.Button(
                        t("training.preset_medium"),
                        variant="secondary",
                    )
                    preset_high_btn = gr.Button(
                        t("training.preset_high"),
                        variant="secondary",
                    )
                ...
                with gr.Row():
                    resume_from = gr.Textbox(
                        label=t("training.resume_from_label"),
                        placeholder="./lora_output/checkpoints/epoch_150",
                        info=t("training.resume_from_info"),
                    )
                ...
                preset_low_btn.click(
                    fn=lambda: (1, 8, 2, 2, False, True, "high", True, False),
                    outputs=preset_targets,
                )
                preset_medium_btn.click(
                    fn=lambda: (2, 4, 4, 4, True, True, "high", True, True),
                    outputs=preset_targets,
                )
                preset_high_btn.click(
                    fn=lambda: (4, 2, 8, 4, True, True, "highest", True, True),
                    outputs=preset_targets,
                )
    ...
    return {
        ...
        "reset_lyrics_btn": reset_lyrics_btn,
        "edit_bpm": edit_bpm,
        ...
        "raw_custom_tag": raw_custom_tag,
        "raw_tag_position": raw_tag_position,
        ...
        "preset_low_btn": preset_low_btn,
        "preset_medium_btn": preset_medium_btn,
        "preset_high_btn": preset_high_btn,
