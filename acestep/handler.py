...
class AceStepHandler:
    ...
    def __init__(self):
        ...
        # Source-audio cache (for reuse across encode/code conversion)
        self._src_audio_cache_key = None
        self._src_audio_cache_audio = None
        self._src_audio_cache_latents = {}  # map: num_frames -> latents (CPU)
        self._src_audio_cache_codes = None
    ...
    def load_lora(self, lora_path: str) -> str:
        ...
        def _resolve_lora_dir(path: str) -> Optional[str]:
            ...
            if _has_adapter_files(path):
                return path
            # Look for adapter folder within path (limited depth)
            base_depth = path.count(os.sep)
            for root, dirs, files in os.walk(path):
                if root.count(os.sep) - base_depth > 2:
                    dirs[:] = []
                    continue
                if "adapter_config.json" in files and (
        ...
        resolved_path = _resolve_lora_dir(lora_path)
        if resolved_path is None:
            return (
                "❌ Invalid LoRA adapter: adapter_config.json + adapter_model.safetensors/bin "
                f"not found under {lora_path}"
            )
    ...
    def set_lora_scale(self, scale: float) -> str:
        ...
        # Iterate through LoRA layers and set their scaling
        try:
            try:
    ...
    def initialize_service(
        ...
        try:
            ...
            # Always ensure base DiT model is available alongside turbo
            success, msg = ensure_dit_model("acestep-v15-base", checkpoint_path, prefer_source=prefer_source)
            if not success:
                return f"❌ Failed to download DiT model 'acestep-v15-base': {msg}", False
            logger.info(f"[initialize_service] {msg}")
    ...
    def _normalize_audio_to_stereo_48k(self, audio: torch.Tensor, sr: int) -> torch.Tensor:
        ...
        if sr != 48000:
            try:
                audio = torchaudio.transforms.Resample(sr, 48000, dtype=audio.dtype)(audio)
            except TypeError:
                # Fallback for older torchaudio without dtype param
    ...
    def _load_audio_file(self, audio_file) -> Optional[Tuple[torch.Tensor, int]]:
        """Load audio using soundfile/ffmpeg to avoid torchcodec DLL issues."""
        try:
            import soundfile as sf

            def _read_with_soundfile(path: str) -> Tuple[torch.Tensor, int]:
                data, sr = sf.read(path, always_2d=True, dtype="float32")
                audio = torch.from_numpy(data).transpose(0, 1)  # [channels, frames]
                return audio, sr

            ext = os.path.splitext(str(audio_file))[1].lower()
            ...
            if ffmpeg_path and ext in {".mp3", ".m4a", ".aac", ".ogg", ".opus", ".wma"}:
                ...
                try:
                    subprocess.run(
                        [ffmpeg_path, "-y", "-i", str(audio_file), "-ac", "2", "-ar", "48000", tmp_path],
                        check=True,
            ...
            return _read_with_soundfile(str(audio_file))
        except Exception as e:
            logger.warning(f"[load_audio_file] soundfile failed, trying torchaudio: {e}")
            try:
                audio, sr = torchaudio.load(audio_file)
                return audio, sr
            except Exception as e2:
                logger.exception(f"[load_audio_file] torchaudio failed: {e2}")
                return None

    def _get_audio_cache_key(self, audio_file) -> Optional[str]:
        """Return a stable cache key for a local audio file input."""
        if audio_file is None:
            return None
        ...
        if isinstance(audio_file, str):
            path = audio_file
        elif isinstance(audio_file, (list, tuple)) and audio_file:
            path = audio_file[0]
    ...
    def _update_src_audio_cache(self, cache_key: Optional[str], audio: Optional[torch.Tensor] = None) -> None:
        """Reset or update source-audio cache for a new key."""
        if cache_key and cache_key == self._src_audio_cache_key:
            if audio is not None:
                self._src_audio_cache_audio = audio
            return

        self._src_audio_cache_key = cache_key
        self._src_audio_cache_audio = audio
        self._src_audio_cache_codes = None
        self._src_audio_cache_latents = {}
    ...
    def process_reference_audio(self, audio_file) -> Optional[torch.Tensor]:
        ...
        try:
            # Load audio file
            loaded = self._load_audio_file(audio_file)
            if loaded is None:
                return None
            audio, sr = loaded
    ...
    def process_src_audio(self, audio_file) -> Optional[torch.Tensor]:
        ...
        try:
            cache_key = self._get_audio_cache_key(audio_file)
            if cache_key and cache_key == self._src_audio_cache_key and self._src_audio_cache_audio is not None:
                return self._src_audio_cache_audio
            ...
            # Normalize to stereo 48kHz
            audio = self._normalize_audio_to_stereo_48k(audio, sr)
            self._update_src_audio_cache(cache_key, audio)
    ...
    def convert_src_audio_to_codes(self, audio_file) -> str:
        ...
        try:
            cache_key = self._get_audio_cache_key(audio_file)
            if cache_key and cache_key == self._src_audio_cache_key and self._src_audio_cache_codes:
                return self._src_audio_cache_codes
            ...
            with torch.no_grad():
                with self._load_model_context("vae"):
                    ...
                    if cache_key:
                        self._update_src_audio_cache(cache_key, processed_audio)
                        self._src_audio_cache_latents[processed_audio.shape[-1]] = latents.detach().cpu()
                ...
                with self._load_model_context("model"):
                    ...
                    if cache_key:
                        self._src_audio_cache_codes = codes_string
                    return codes_string
    ...
    def _prepare_batch(
        ...
        src_cache_key: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
            ...
            with self._load_model_context("vae"):
                for i in range(batch_size):
                    ...
                    if self.is_silence(current_wav):
                        ...
                        cached_latents = None
                        if src_cache_key and src_cache_key == self._src_audio_cache_key:
                            cached_latents = self._src_audio_cache_latents.get(current_wav.shape[-1])
                        if cached_latents is not None:
                            logger.info(f"[generate_music] Reusing cached source latents for item {i}...")
                            target_latent = cached_latents.to(self.device).to(self.dtype)
                        else:
                            ...
                            if src_cache_key:
                                self._update_src_audio_cache(src_cache_key)
                                self._src_audio_cache_latents[current_wav.shape[-1]] = target_latent.detach().cpu()
    ...
    def generate_music(
        ...
        try:
            ...
            src_cache_key = self._get_audio_cache_key(src_audio) if src_audio is not None else None
