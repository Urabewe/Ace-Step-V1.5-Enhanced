...
if defined VIRTUAL_ENV (
    if exist "%VIRTUAL_ENV%\Scripts\python.exe" (
        echo [Environment] Using active virtual environment...
...
if exist "%~dp0.venv\Scripts\python.exe" (
    echo [Environment] Using local .venv...
...
REM Offer to create a local venv if missing
if not exist "%~dp0.venv\Scripts\python.exe" if not exist "%~dp0venv\Scripts\python.exe" (
    if exist "%~dp0install_Ace-Step_WIN.bat" (
        echo [Setup] No local venv found.
        set /p INSTALL_VENV="Create .venv now? (Y/N): "
        if /i "!INSTALL_VENV!"=="Y" (
            call "%~dp0install_Ace-Step_WIN.bat"
            goto :TryLocalVenv
        )
    )
)
